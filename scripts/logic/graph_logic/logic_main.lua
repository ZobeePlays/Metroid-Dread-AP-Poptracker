...
metroid_dread_location = {}
metroid_dread_location.__index = metroid_dread_location
...
function metroid_dread_location.new(name)
    local self = setmetatable({}, metroid_dread_location)
    if name then
...
function metroid_dread_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = metroid_dread_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function metroid_dread_location:connect_two_ways(exit, rule)
...
function metroid_dread_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function metroid_dread_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function metroid_dread_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function metroid_dread_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function metroid_dread_location:accessibility()
...
-- 
function metroid_dread_location:discover(accessibility, keys)
...
entry_point = metroid_dread_location.new("entry_point")
